...
def main():
    ...
    if args.tokenizer_name:
        tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name,from_diffusers = args.from_diffusers)
    elif args.pretrained_model_name_or_path:
        tokenizer = AutoTokenizer.from_pretrained(url_or_path_join(args.pretrained_model_name_or_path, "tokenizer"),from_diffusers = args.from_diffusers)

    # import correct text encoder class
    text_encoder_cls = import_model_class_from_model_name_or_path(args.pretrained_model_name_or_path,from_diffusers = args.from_diffusers,)

    # Load scheduler and models
    noise_scheduler = DDPMScheduler.from_pretrained(args.pretrained_model_name_or_path, subfolder="scheduler",from_diffusers = args.from_diffusers,)
    ...
    vae = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path, subfolder="vae",from_diffusers = args.from_diffusers,)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        unwrap_model(unet).save_pretrained(os.path.join(save_path, "unet"),to_diffusers=args.to_diffusers)
                        if args.train_text_encoder:
                            unwrap_model(text_encoder).save_pretrained(os.path.join(save_path, "text_encoder"),to_diffusers=args.to_diffusers)
    ...
    if is_main_process:
        ...
        pipeline.save_pretrained(args.output_dir,to_diffusers=args.to_diffusers)
