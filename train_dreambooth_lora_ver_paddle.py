...
def import_model_class_from_model_name_or_path(pretrained_model_name_or_path: str, from_diffusers: bool, from_hf_hub: bool):
    try:
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--to_pytorch_safetensors", action="store_true", help="Whether or not to save as pytorch safetensors."
    )
    parser.add_argument(
        "--from_diffusers", action="store_true", help="Whether or not the base model is diffusers model."
    )
    parser.add_argument(
        "--from_hf_hub", action="store_true", help="Whether or not to download model from hf_hub."
    )
...
def main():
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            pipeline = DiffusionPipeline.from_pretrained(
                ...
                safety_checker=None,
                from_diffusers = args.from_diffusers,
    ...
    if args.tokenizer_name:
        tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name,from_diffusers = args.from_diffusers,from_hf_hub = args.from_hf_hub)
    elif args.pretrained_model_name_or_path:
        tokenizer = AutoTokenizer.from_pretrained(url_or_path_join(args.pretrained_model_name_or_path, "tokenizer"),from_diffusers = args.from_diffusers,from_hf_hub = args.from_hf_hub)

    # import correct text encoder class
    text_encoder_cls = import_model_class_from_model_name_or_path(args.pretrained_model_name_or_path, args.from_diffusers, args.from_hf_hub)

    # Load scheduler and models
    noise_scheduler = DDPMScheduler.from_pretrained(args.pretrained_model_name_or_path, subfolder="scheduler",from_diffusers = args.from_diffusers,from_hf_hub = args.from_hf_hub)
    ...
    vae = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path, subfolder="vae",from_diffusers = args.from_diffusers,from_hf_hub = args.from_hf_hub)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        unwrap_model(unet).save_attn_procs(save_path, to_diffusers=args.to_diffusers)
    ...
    if is_main_process:
        unet = unwrap_model(unet)
        unet.save_attn_procs(args.output_dir, to_diffusers=args.to_diffusers)

        # Final inference
        # Load previous pipeline
        pipeline = DiffusionPipeline.from_pretrained(args.pretrained_model_name_or_path, safety_checker=None, from_diffusers = args.from_diffusers, from_hf_hub = args.from_hf_hub)
        pipeline.scheduler = DPMSolverMultistepScheduler.from_config(pipeline.scheduler.config)
